...
var docker = {
	...
	run : function() {
		...
		let runContainer = `sudo docker run ${save} -it -l epidocker_flag=true -l epidocker_image=${opts.image} -v ${project.directory}:${remoteDirectory} --name=${project.name} ${opts.image} ${defaultCmd}`;
