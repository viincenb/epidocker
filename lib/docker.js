...
var docker = {
	...
	run : function() {
		...
		let env = "-e TERM -e EPIDOCKER_PROJECT_DIRECTORY=" + remoteDirectory;
		...
		if (opts.graphic) {
			...
			if (process.env === 'darwin') {
				let ip = '$(ifconfig en0 | grep inet | awk \'$1=="inet" {print $2}\')';
				baseCmd = 'xhost + ' + ip + ' && ' + baseCmd;
				DISPLAY = ip + ':0';
			} else {
				baseCmd = 'xhost +local:root > /dev/null && ' + baseCmd;
			}
		    env += " --device /dev/dri --device /dev/snd -e DISPLAY=" + DISPLAY;
		    env += " -v /run/user/1000/pulse:/run/user/1000/pulse -e PULSE_SERVER=unix:/run/user/1000/pulse/native";
		}

		if (opts.expose !== '')
			dockerFlags += ' -p ' + opts.expose;
