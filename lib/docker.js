...
var docker = {
	...
	run : function() {
		let defaultCmd = 'bash';
		let runContainer = `sudo docker run -it -v ${project.directory}:${opts.remote}/${project.name} --name=${project.name} ${opts.image} ${defaultCmd}`;
	...
	list : function() {
		...
		exec(cmd, (error, stdin, stderr) => {
			...
			if (containers.length === 0) {
				console.log('You do not have any containers');
				...
				console.log('\nType "epidocker run [container name]" to access a container');
			}
		});
	},
