...
manage = {
	...
	init : function() {
		args
			...
			.option('verbose'	, 'Verbose mode'												, opts.verbose)
			...
			.option('name'		, 'Container name to create'									, opts.containerName, ['n'])
			.option('expose'	, 'Expose every container ports to host'						, opts.expose)
	...
	execArg : function(name, sub, options) {
		...
		opts.containerName = options.name;
		opts.verbose = options.verbose;
		opts.expose = options.expose;
